...
void merge(int* arr, int* temp, int left, int mid, int right) {    int i = left;
    int j = mid + 1;
    int k = left;
...
void fill_table_1() {
    ...
    for (int n = step; n <= max_n; n += step) {
        ...
        for (int iter = 0; iter < iterations; iter++) {
            double start = wtime();
            linear_search(arr, n, key);            t_linear += wtime() - start;
        }
        t_linear = (t_linear / iterations) * 1000000.0;
...
int main() {
    ...
    printf("Avg linear search time:   %.4f us\n", t_linear_single);    printf("Avg binary search time:   %.4f us\n", t_binary_single);
